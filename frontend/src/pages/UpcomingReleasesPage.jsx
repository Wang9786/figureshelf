...
function UpcomingReleasesPage() {
  ...
  async function fetchItems(targetDays = days) {
    ...
    try {
      const response = await apiClient.get(`/figures/upcoming-releases?days=${targetDays}`)
    ...
    <div className="min-h-screen bg-slate-100">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
          <div>
            ...
            <p className="text-sm text-slate-500">即將發售</p>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <h2 className="text-xl font-bold text-slate-900">即將發售</h2>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl bg-white p-10 text-center shadow-sm">
            <p className="text-slate-600">目前沒有即將發售的公仔</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {items.map((figure) => (
                  <tr key={figure.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-4 font-medium text-slate-900">
                      {getDaysLeft(figure.release_date)}
                    </td>
                    <td className="px-5 py-4 text-slate-600">
                      NT$ {figure.price}
                    </td>
